...
def main():
    parser = argparse.ArgumentParser(description="Unified runner for the MIGO project.")
    parser.add_argument("--task", required=True, choices=["scRNA_scHiC", "RNA_Ribo_seq", "scRNA_ADT", "scRNA_scATAC"])
    parser.add_argument("--mode", required=True, choices=["train", "infer"], help="Run mode")
    parser.add_argument("--resolution", choices=["1M", "50K"], help="scRNA_scHiC resolution")
    parser.add_argument("--config", default=None, help="Config path override for any task")
    ...
    run_task(args.task, args.mode, config_path=args.config, resolution=args.resolution, overrides=overrides)
